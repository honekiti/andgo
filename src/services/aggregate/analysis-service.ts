...
export const refreshTotalAmount = async () => {
  ...
  const totalBtcAmount = filtered.reduce((acc, order) => acc + ((order.result as SuccessOrderResult).btcAmount ?? 0), 0);
  const totalSpentAmount = filtered.reduce((acc, order) => acc + (order.planSnapshot.buy?.quoteAmount ?? 0), 0);
